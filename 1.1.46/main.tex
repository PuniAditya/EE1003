...
Consider the AC bridge shown below. $A, R, L,$ and $C$ are positive finite values. The input is $v_{in}(t) = A\sin(\omega t)$. Find the condition for $V_0 = 0$.
\begin{figure}
    \centering
...
\end{figure}
\begin{enumerate}
    \item $\omega L = \frac{1}{\omega C}$
    \item $L = C$
    \item $R = \frac{1}{\omega\sqrt{LC}}$
    \item \textbf{$V_0$ cannot be made zero}
\end{enumerate}
\end{frame}

\begin{frame}{Theoretical Solution: Laplace Domain}
...
\vfill
\textbf{Conclusion:} 
\\ No real frequency $\omega$ can satisfy $\omega^2LC = -1$. Therefore, $V_0$ cannot be zero.
\end{frame}
...
\begin{figure}
	...
	\label{fig:sim}
\end{figure}
The simulation confirms that $V_0(t)$ is a persistent sinusoid, meaning the bridge never reaches a zero-amplitude steady state.
\end{frame}
